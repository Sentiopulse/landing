...
export default function MeetSentioCards() {
	return (
			<div className="w-full max-w-6xl grid md:grid-cols-2 gap-8">
						{MeetSentioPulseCardInfo.map((card) => {
							const Icon = card.icon;
							return (
								<Card key={card.title} className="relative border border-border bg-[var(--custom-card-color)] shadow-2xl backdrop-blur-xl transition duration-200 hover:shadow-[0_0_20px_rgba(0,255,0,0.3)]">
